...
!
! This is a double-precision version of the routine  sti.
!
      double precision dx,dw,dalpha,dbeta,dp0,dp1,dp2,dtiny,dhuge,dsum0,dsum1,dsum2,dt
      dimension dx(ncap),dw(ncap),dalpha(n),dbeta(n),dp0(ncap),dp1(ncap),dp2(ncap)
      dtiny=10.d0*tiny(dtiny)
      dhuge=.1d0*huge(dhuge)
      ierr=0
